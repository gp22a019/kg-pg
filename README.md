...
9. **クリッカブルタイトル**: ページタイトル「Wikidata検索」をクリックしてホームに戻る機能
10. **検索履歴機能**: LocalStorageを使った検索履歴の保存・表示・再実行（通常検索・詳細検索両対応）
9. **タイトルナビゲーション**: 「Wikidata検索」「Wikidata詳細検索」タイトルクリックでホームに戻る機能
...
✅ **ナビゲーション機能（NEW）**
- **クリッカブルタイトル**: ページタイトル「Wikidata検索」クリックでホーム画面に戻る
- **ホバー効果**: タイトルにマウスオーバーで色変化とカーソル変更

✅ **検索履歴機能（NEW）**
- **LocalStorage活用**: ブラウザに検索履歴を永続保存
- **通常検索履歴**: 最新5件表示、検索語・言語・実行時刻を記録
- **詳細検索履歴**: 最新4件表示、検索タイプ・エンティティ種別も記録
- **ワンクリック再実行**: 履歴アイテムクリックで検索条件復元・自動実行
- **個別削除**: 各履歴アイテムに削除ボタン（×）
- **一括削除**: 「履歴クリア」ボタンで全削除（確認ダイアログ付き）
- **重複除去**: 同じ検索条件は最新のものだけ保持
- **件数制限**: 通常検索20件、詳細検索15件まで自動管理
- **アニメーション**: 履歴セクションの表示/非表示にスムーズなアニメーション

✅ **ナビゲーション機能（NEW）**
- **クリッカブルタイトル**: 「Wikidata検索」「Wikidata詳細検索」タイトルクリックでホームに戻る
- **ホバー効果**: タイトルホバー時の視覚的フィードバック
- **統一ナビゲーション**: 全ページで一貫したナビゲーション体験
...
**検索履歴機能**:
1. 何かを検索すると検索入力欄下に履歴セクションが自動表示されます
2. 履歴アイテムをクリック → 検索条件が復元され自動実行されます  
3. 各履歴の「×」ボタンで個別削除、「履歴クリア」で全削除
4. `/advanced` 詳細検索ページでも同様の履歴機能が利用できます

**ナビゲーション機能**:
1. ページ上部の「Wikidata検索」タイトルをクリックするとホーム画面に戻ります
2. 詳細検索ページの「Wikidata詳細検索」タイトルも同様にクリック可能です
...
- **最終更新**: 2025-09-29 (検索履歴機能・ナビゲーション機能追加)
